...
function App() {
  ...
  return (
    <>
      ...
      <h1 className='font-bold text-xl text-center'>Let's Go</h1>
        <div className="addTodo my-5 flex flex-col gap-4">
          <h2 className='text-lg font-bold text-center'>Add a Todo</h2>
          <input type="text" className='w-full placeholder:text-black placeholder:text-bold placeholder:text-center hover:border border-blue-800 rounded-lg py-1 px-3'  placeholder='Enter Task Here' onChange={handleChange} value={todo} />
        ...
        <h2 className='text-lg font-bold max-lg:text-center my-2'>My Tasks</h2>
        <div className="todos">
            ...
            return (showFinshed || !items.isCompleted) && <div key={items.id} className="todo flex w-full lg:w-1/2 justify-between my-3">
              <div className='flex gap-5'>
                ...
                <label htmlFor={items.id}>
                  <div className={items.isCompleted ? "line-through break-all" : "break-all"}>{items.todo}</div>
                </label></div>
              <div className="buttons flex h-full">
                <button onClick={(e) => { handleEdit(e, items.id) }} className='bg-violet-800 hover:bg-violet-950 p-2 py-1 rounded-md text-sm text-white mx-1'><FaEdit /></button>
                <button onClick={(e) => { handleDelete(e, items.id) }} className='bg-violet-800 hover:bg-violet-950 p-2 py-1 rounded-md text-md text-white mx-1'><MdDeleteOutline /></button>
              </div>
            </div>
          })}
        </div>
